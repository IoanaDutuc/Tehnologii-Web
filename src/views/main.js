...
const server = http.createServer((req, res) => {
    ...
    if (reqUrl.pathname === '/auth' && req.method === 'POST') {
        ...
        req.on('end', async () => {
            ...
            if (requestData.requestType === 'login') {
                ...
                try {
                    connection.query('SELECT * FROM user WHERE (email = ? OR username = ?) AND password = ?', [email, email, hashedPassword], (error, results, fields) => {
                        if (error) {
                ...
                try {
                    connection.query('SELECT * FROM user WHERE email = ? OR username = ?', [email, username], async (error, results, fields) => {
                        if (error) {
                            res.writeHead(500, {'Content-Type': 'text/plain'});
                            res.end('Internal server error');
                        }
                        if (results.length !== 0) {
                            ...
                            await connection.query('INSERT INTO user (email, firstname, lastname, username, birth, password) VALUES (?, ?, ?, ?, ?, ?)', [email, firstName, lastName, username, formattedbirth, hashedPassword]);
                            const token = generateToken(email);
                            res.writeHead(200, {'Content-Type': 'application/json'});
                            res.end(JSON.stringify({token: token}));
                        }
    ...
    if (reqUrl.pathname === '/profileEdit/' && req.method === 'PUT') {
        ...
        req.on('end', async () => {
            ...
            if (photo === '')
                photo = null;
            try {
                connection.query('SELECT * FROM user WHERE email != ? AND username = ?', [email, username], async (error, results, fields) => {
                    if (error) {
                        res.writeHead(500, {'Content-Type': 'text/plain'});
                        res.end('Internal server error');
                    }
                    if (results.length !== 0) {
                        ...
                        await connection.query('UPDATE user SET username = ?, firstname = ?, lastname = ?, birth = ?, photo = ? WHERE email = ?', [username, firstName, lastName, formattedbirth, photo, email]);
                        res.writeHead(200, {'Content-Type': 'application/json'});
    ...
    if (reqUrl.pathname === '/profilePassword/' && req.method === 'PUT') {
        ...
        req.on('end', async () => {
            ...
            try {
                connection.query('SELECT * FROM user WHERE email = ? AND password = ?', [email, hashedPassword], async (error, results, fields) => {
                    if (error) {
                        res.writeHead(500, {'Content-Type': 'text/plain'});
                        res.end('Internal server error');
                    }
    ...
    if (reqUrl.pathname === '/profileDelete/' && req.method === 'DELETE') {
        ...
        req.on('end', async () => {
            const email = body;
            try {
                connection.query('DELETE FROM user WHERE email = ?', [email], async (error, results, fields) => {
                    if (error) {
                        res.writeHead(500, {'Content-Type': 'text/plain'});
                        res.end('Internal server error');
                    } else {
        ...
        try {
            ...
            for (let filter of filterArray) {
                if (filter === filterArray[0])
                    ...
                    query += ("OR");
                query += (" material LIKE '%" + filter + "%' ");
                query += ("OR");
                query += (" type LIKE '%" + filter + "%' ");
                query += ("OR");
                query += (" brand LIKE '%" + filter + "%' ");
                query += ("OR");
                query += (" occasion LIKE '%" + filter + "%' ");
                query += ("OR");
                query += (" color LIKE '%" + filter + "%' ");
            }
    ...
    // ----------------------------------------- Favorite ---------------------------------------------------
    if (reqUrl.pathname.startsWith('/shoe/favorite') && req.method === 'POST') {
        let body = '';
        req.on('data', (chunk) => {
            body += chunk;
        });
        req.on('end', async () => {
            const requestData = JSON.parse(body);
            const email = requestData.email;
            const shoeId = requestData.shoeId;
            console.log('Received data:', email);
            console.log('Received data:', shoeId);
            try {
                connection.query('SELECT id FROM user WHERE email = ?', [email], async (error, results, fields) => {
                    if (error) {
                        res.writeHead(500, {'Content-Type': 'text/plain'});
                        res.end('Internal server error');
                        ...
                        await connection.query('INSERT INTO favorite (idUser, idShoe) VALUES (?, ?)', [idUser, shoeId]);
                    }
                });
            } catch (error) {
                res.writeHead(500, {'Content-Type': 'text/plain'});
    ...
    if (reqUrl.pathname === '/shoes/favorite/page' && req.method === 'POST') {
        let body = '';
        req.on('data', (chunk) => {
            body += chunk;
        });
        req.on('end', () => {
            const email = body;
            const query = "SELECT shoe.* FROM shoe JOIN favorite ON shoe.id = favorite.idShoe JOIN user ON favorite.idUser = user.id WHERE user.email = ?";
            console.log(query);
            connection.query(query, [email], (error, results, fields) => {
                ...
                if (results.length === 0) {
                    ...
                    res.end('Nu ai produse la favorite');
                    return;
                }
    ...
    if (reqUrl.pathname.startsWith('/shoe/favorite/delete') && req.method === 'DELETE') {
        let body = '';
        req.on('data', (chunk) => {
            body += chunk;
        });
            ...
            try {
                connection.query('SELECT id FROM user WHERE email = ?', [email], async (error, results, fields) => {
                    if (error) {
                        ...
                        console.log(idUser);
                        console.log('Received data:', shoeId);
                        await connection.query('DELETE FROM favorite WHERE idUser = ? AND idShoe = ?', [idUser, shoeId]);
                    }
                });
            } catch (error) {
                res.writeHead(500, {'Content-Type': 'text/plain'});
